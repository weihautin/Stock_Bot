...
for n in owner.keys():
    if owner[n][-1][-4] > owner[n][-2][-4] > owner[n][-3][-4]:
        print n,'大股東持股數持續增加',float(owner[n][-1][-4])-float(owner[n][-3][-4])
        big_tmp.append(n)
...
for m in owner.keys():
    try:
        if float(owner[m][-1][-3].replace(',', '')) > float(owner[m][-2][-3].replace(',', '')) > float(owner[m][-3][-3].replace(',', '')):
            print m,'股東越來越多',float(owner[m][-1][-3].replace(',', ''))-float(owner[m][-3][-3].replace(',', ''))
            more_tmp.append(m)
    except:
        pass
...
for m in owner.keys():
    try:
        if float(owner[m][-1][-3].replace(',', '')) < float(owner[m][-2][-3].replace(',', '')) < float(owner[m][-3][-3].replace(',', '')):
            print m,'股東越來越少',float(owner[m][-3][-3].replace(',', ''))-float(owner[m][-1][-3].replace(',', ''))
            less_more.append(m)
    except:
        pass
...
for n in owner.keys():
    try:
        if owner[n][-1][-4] < owner[n][-2][-4] < owner[n][-3][-4]:
            print n,'大股東持股數持續減少',float(owner[n][-3][-4])-float(owner[n][-1][-4])
