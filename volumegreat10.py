...
title = str(time_now+"增10倍") #Email郵件的標題 ex:2015-0411-選股機器人

day = 20

#attachment = str(time_now)+'-'+str(day)+'.txt' #附件名稱使用當日時間 ex:2015-0411.txt

fileopen = open("volumegreat10.txt", 'w') #開啟檔案,w沒有該檔案就新增
...
for i in OTC_no_list:
    #print i
    try:
        if BestFourPoint(Stock(i,mons=1)).volumegreat10():
           print i,'otc'         #暴量長紅2天
...
os.system('sendEmail -o \
 ...
 -xp %s \
 -u %s \
 -o message-file=/home/tim/volumegreat10.txt \
 '%(ID, PW, title))
