...
realtime_txt.write(year+"年第一季EPS"+"\n")
...
with open('TWSE.csv') as csvfile:
    ...
    for row in readCSV: #就readCSV裡的所有資料(以列為單位)
        ...
        'encodeURIComponent=1&step=1&firstin=1&off=1&keyword4=&code1=&TYPEK2=&checkbtn=&queryName=co_id&TYPEK=all&isnew=false&co_id='+row[0]+'&year='+year+'&season=01'
            ...
            for tr in trs:
                ...
                for td in tds:
                    if (td.get_text().strip()==u"基本每股盈餘") :
                        ...
                        if (tds[1].get_text().strip()!=''):
                            realtime_txt.write(row[0]+" "+tds[1].get_text().strip()+" "+tds[3].get_text().strip()+" "+tds[5].get_text().strip()+" "+tds[7].get_text().strip()+" "+row[2]+"\n")
