...
attachment = str(time_now)+'-realtimefilter'+'.csv' #附件名稱使用當日時間 ex:2015-0411.txt

fileopen = open(attachment,'w')
...
for i in stock_no_list:
    ...
    try:
        if realtime_data.data[i]['volume_acc'] > Stock(i,mons=1).moving_average(5)[0][-1]: #今天的量大於5日週均量
           ...
           #fileopen.write(','+str(i[1])+','+str(contribution)+','+ str(total)+"\n")
           #fileopen.write("時間"+time_hour+Stock_no_name[i].encode("UTF-8")+"-"+i+"\n")
           #fileopen.write(Stock_no_name[i].encode("UTF-8")+"\n")
